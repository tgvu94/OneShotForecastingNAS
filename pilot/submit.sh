...
#   pilot/submit.sh train   [ROOT] [DATASET] [HORIZON]   -> ground-truth runner, job array: 6 tasks, at most 2 running, 8 h each (<= 48 GPU-h)
...
case "$mode" in
  train)   name=stzero;  extra=(--array=0-5%2 --time=08:00:00 --output="$HOME/nas/logs/stzero_${tag}_%A_%a.log");  script=pilot/run_train_all.sbatch ;;
